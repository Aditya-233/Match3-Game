...
int main(void) {
    ...
    if (swapSnd.stream.buffer != nullptr) SetSoundVolume(swapSnd, 0.35f);
    if (matchSnd.stream.buffer != nullptr) SetSoundVolume(matchSnd, 1.0f);
                ...
                if (selR < 0) {
                    ...
                    if (std::abs(mc - selC) + std::abs(mr - selR) == 1) {
                        ...
                        do {
                            ...
                            for (int i = 0; i < G; i++)
                                for (int j = 0; j < G - 2; j++) {
                                    if (board[i][j] >= 0 && board[i][j] == board[i][j + 1] && board[i][j] == board[i][j + 2]) m[i][j] = m[i][j + 1] = m[i][j + 2] = matchFound = true;
                                    if (board[j][i] >= 0 && board[j][i] == board[j + 1][i] && board[j][i] == board[j + 2][i]) m[j][i] = m[j + 1][i] = m[j + 2][i] = matchFound = true;
                                }
                            if (matchFound) {
                                for (int r = 0; r < G; r++)
                                    for (int c = 0; c < G; c++)
                                ...
                                if (matchSnd.stream.buffer != nullptr) PlaySound(matchSnd);
                                pass++;
                            }
                        } while (matchFound);
                        if (pass == 0) {
                            std::swap(board[selR][selC], board[mr][mc]);
                            if (swapSnd.stream.buffer != nullptr) PlaySound(swapSnd);
                        }
                        selR = selC = -1;
                    } else {
                        selR = mr;
        ...
        for (int r = 0; r < G; r++)
            for (int c = 0; c < G; c++) {
                ...
                if (t >= 0 && t < 5) {
                    ...
                    if (gems[idx].id > 0) {
                        Rectangle srcRec{0.0f, 0.0f, static_cast<float>(gems[idx].width), static_cast<float>(gems[idx].height)};
                        Rectangle destRec{static_cast<float>(ox + c * cs + 2), static_cast<float>(oy + r * cs + 2), static_cast<float>(cs - 4), static_cast<float>(cs - 4)};
                        DrawTexturePro(gems[idx], srcRec, destRec, Vector2{0.0f, 0.0f}, 0.0f, WHITE);
                    }
                }
            }

        if (selR >= 0 && selC >= 0) DrawRectangleRoundedLinesEx(Rectangle{static_cast<float>(ox + selC * cs + 2), static_cast<float>(oy + selR * cs + 2), static_cast<float>(cs - 4), static_cast<float>(cs - 4)}, 0.15f, 4, 3.0f, Color{241, 196, 15, 255});
    ...
    for (int i = 0; i < 5; i++)
        if (gems[i].id > 0) UnloadTexture(gems[i]);
    if (bg.id > 0) UnloadTexture(bg);
    if (font.texture.id > 0) UnloadFont(font);
    if (bgm.stream.buffer != nullptr) UnloadMusicStream(bgm);
    if (swapSnd.stream.buffer != nullptr) UnloadSound(swapSnd);
    if (matchSnd.stream.buffer != nullptr) UnloadSound(matchSnd);
